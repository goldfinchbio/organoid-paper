...
matrix_barplot = function(data, group_by=NULL, pvals=NULL, xlab='', ylab='Frequency', value='mean', error='se', legend.title='Groups', colors='Paired', pos='dodge', border=NA,
                          out=NULL, nrow=1.5, ncol=1.5, coord_flip=FALSE, sig_only=F, do.facet=F, asterix.size=5){
    ...
    # Plot data
    if(pos == 'stack'){
        if(error %in% c('sd', 'se')){p = ggplot(data) + geom_errorbar(aes(x=x, ymin=u-s, ymax=u+s, fill=group), stat='identity', width=.25)}
        p = p + geom_bar(aes(x=x, y=u, fill=group), colour=border, size=.25, stat='identity')
    } else {
        pos = position_dodge(.9)
        if(error %in% c('sd', 'se')){p = ggplot(data) + geom_errorbar(aes(x=x, ymin=u-s, ymax=u+s, fill=group), stat='identity', position=pos, width=.25)}
        p = p + geom_bar(aes(x=x, y=u, fill=group), colour=border, size=.25, stat='identity', position=pos)
    }
	...
	if(!is.null(pvals)){p = p + geom_text(aes(x=x, y=u+s+dy, label=lab1, group=group), hjust='center', vjust=0, size=asterix.size, angle=0, position=pos)}
    } else {
        p = p + coord_flip()
	if(!is.null(pvals)){p = p + geom_text(aes(x=x, y=u+s+dy, label=lab1, group=group), hjust='center', vjust=1, size=asterix.size, angle=90, position=pos)}
    }
