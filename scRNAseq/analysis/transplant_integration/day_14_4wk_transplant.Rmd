---
title: "Day 14 - 4 wk transplant integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
annotations <- annotations %>% filter(grepl("14_4wk", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 14 - 4wk transplant** samples. 
...
Clusters 0 and 10 have high ribosomal transcripts
...
seur <- subset(seur, RNA_snn_res.0.5 != 0 & RNA_snn_res.0.5 != 10)
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vivo/day_14_4_wk_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
