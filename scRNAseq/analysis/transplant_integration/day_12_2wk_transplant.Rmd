---
title: "Day 12 - 2 wk transplant integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
annotations <- annotations %>% filter(grepl("12_2wk", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 12 - 2wk transplant** samples. 
...
The high ribosomal population is in Clusters 0 and 1
...
seur <- subset(seur, RNA_snn_res.1 != 0 & RNA_snn_res.1 != 1)
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vivo/day_12_2_wk_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
