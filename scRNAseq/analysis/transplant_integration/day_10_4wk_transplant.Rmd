---
title: "Day 10 - 4 wk transplant integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
annotations <- annotations %>% filter(grepl("10_4wk", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 10 - 4wk transplant** samples. 
...
The high ribosomal population is in Clusters 0, 3, 9 and 10
...
seur <- subset(seur, RNA_snn_res.0.5 != 0 & RNA_snn_res.0.5 != 3 & RNA_snn_res.0.5 != 9 & RNA_snn_res.0.5 != 10)
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vivo/day_10_4_wk_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
...
seur$cell_type_simple_2 <- seur$cell_type_simple

Idents(seur) <- "cell_type_manual"

seur$cell_type_manual_2 <- seur$cell_type_manual
