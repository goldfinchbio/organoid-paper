---
title: "Day 12 - 4 wk transplant integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
annotations <- annotations %>% filter(grepl("12_4wk", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 12 - 4wk transplant** samples. 
...
seur <- ScaleData(seur, vars.to.regress = c("nCount_RNA", "percent.mito", "S.Score", "G2M.Score"),
                      verbose = TRUE)
...
The high ribosomal population is in Clusters 0, 2, and 14

```{r filter-ribo}
DimPlot(seur, group.by = "RNA_snn_res.1", label = T)
...
seur <- subset(seur, RNA_snn_res.1 != 0 & RNA_snn_res.1 != 2 & RNA_snn_res.1 != 14)
...
```{r cc_umap, cache=FALSE, eval=do_eval}
...
```{r clusters, results="hide", eval=do_eval, cache=FALSE}
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vivo/day_12_4_wk_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
...
colors <- c(gg_color_hue(9), "grey90", "grey80", "grey70", "grey60", "grey50", "grey40", "grey30", "grey20", "grey10")
...
if(file.exists(file.path(saveDir, "dt_seur.RDS"))){
  ...
  res = seq(0,1,0.1)
  dt_seur <- FindClusters(dt_seur, resolution = 3)
...
DimPlot(dt_seur, group.by="RNA_snn_res.3", label = T)
```
...
```{r, fig.height=6}
VlnPlot(dt_seur, features = c("AQP2", "CALB1", "CDH1", "KRT8"), group.by = "RNA_snn_res.3", ncol = 2, pt.size = 0.1)
...
```{r, fig.height=6}
VlnPlot(dt_seur, features = vln_markers_list[["Distal Tubule"]]$Gene, group.by = "RNA_snn_res.3", ncol = 2, pt.size = 0.1)
```
...
dt_seur$cell_type_manual <- ifelse(dt_seur$RNA_snn_res.3 %in% cd_clusters, "Collecting Duct", "Distal Tubule")
...
I chose clusters **`r cd_clusters`** for collecting duct
