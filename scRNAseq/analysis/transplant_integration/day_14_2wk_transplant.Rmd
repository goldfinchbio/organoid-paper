---
title: "Day 14 - 2 wk transplant integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
annotations <- annotations %>% filter(grepl("14_2wk", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 14 - 2wk transplant** samples. 
...
The high ribosomal population is in Cluster 0
...
# seur <- CellCycleScoring(seur, s.features = cc.genes$s.genes, g2m.features = cc.genes$g2m.genes, assay = "RNA")
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vivo/day_14_2_wk_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
...
if(file.exists(file.path(saveDir, "dt_seur.RDS"))){
  ...
  dt_seur$RNA_snn_res.1.4 <- factor(dt_seur$RNA_snn_res.1.4, levels = min(as.numeric(levels(dt_seur$RNA_snn_res.1.4))):max(as.numeric(levels(dt_seur$RNA_snn_res.1.4))))
...
dt_seur$cell_type_manual <- ifelse(dt_seur$RNA_snn_res.1.4 %in% cd_clusters, "Collecting Duct", "Distal Tubule")
