---
title: "Day 12 in vitro integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
DOCNAME = "Day_12_in_vitro_integration"
...
annotations <- annotations %>% filter(grepl("day12", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 12 *in vitro* ** samples. 
...
Clusters 0 and 6 have high ribosomal transcripts
...
seur <- subset(seur, RNA_snn_res.0.5 != 0 & RNA_snn_res.0.5 != 6)
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vitro/day12_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
