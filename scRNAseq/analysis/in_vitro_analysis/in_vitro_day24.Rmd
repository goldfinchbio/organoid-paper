---
title: "Day 24 in vitro integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
DOCNAME = "Day_24_in_vitro_integration"
...
seq_dirs <- list.files("/mnt/data/data2", recursive = F, include.dirs = T, pattern = "WIGTC", full.names = T)
...
annotations <- annotations %>% filter(grepl("day24", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 24 *in vitro* ** samples. 
...
Cluster 0 has high ribosomal transcripts
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vitro/day24_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
