---
title: "Day 7 in vitro integration"
output:
  html_document:
    code_folding: hide
    collapsed: no
...
DOCNAME = "Day_07_in_vitro_integration"
...
annotations <- annotations %>% filter(grepl("day7", sample_name))
```
...
In this analysis we will perform integration, clustering and cell type identification of the 3 **Day 7 *in vitro* ** samples. 
...
Clusters 3 and 7 have high ribosomal transcripts
...
seur <- subset(seur, RNA_snn_res.0.5 != 3 & RNA_snn_res.0.5 != 7)
...
cluster_id <- read_csv(here("data/cluster_IDs/in_vitro/day07_2.csv")) %>% mutate(Cluster = as.factor(Cluster))
